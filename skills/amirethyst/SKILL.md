---
name: amirethyst
description: An inspired interpretation of Amirethyst's design language — a developer-focused AI agent engineering platform whose surface is an unrelenting near-black canvas broken only by a single amirethyst brand accent, code-editor mockups inside the hero, and a precise grid of dark feature cards that read like a documentation site dressed as marketing.
...
Amirethyst is an AI agent engineering platform built for developers, and the brand wears that audience proudly: a deep indigo-black `{colors.canvas}` (`#1F1F28`) page background that runs edge-to-edge with no light-mode counterpart, a single amirethyst accent (`{colors.primary}` `#A387D5`) reserved for CTAs, status pills, and key highlights, and a typography system that pairs sentence-case Inter with SF Mono for inline code and command snippets. The whole page reads like polished documentation that decided to also sell something.
`
The decorative system is restrained. There is no gradient mesh, no atmospheric backdrop, no illustration suite. Instead, the brand uses small typographic moments — an amirethyst code chip (`npx amirethyst ...`), a 3-px outlined feature card sitting against the same near-black canvas, an accent hairline divider between section bands — to mark its identity. The result is a page that feels engineered: every card has a hairline border, every snippet has a copy-to-clipboard button, every metric is rendered in a numeric monospace.
...
**Key Characteristics:**
- A single amirethyst accent `{colors.primary}` (`#A387D5`) carries every CTA, every status pill, and key active states. No second accent.
- Dark canvas (`{colors.canvas}` `#1F1F28`) is the only page surface — there is no light-mode rhythm; the entire site reads as one continuous dark surface broken by feature-card boundaries.
- Hairline-bordered feature cards (`{colors.hairline}` `#313244`, 1 px solid) are the brand's primary chrome — no shadows, no fills, just precise hairline rectangles.
...
### Brand & Accent
- **Amirethyst** (`{colors.primary}` — `#A387D5`): The single brand accent. Every primary CTA, every status pill, and the dominant highlight state. Reserved.
- **Primary Soft** (`{colors.primary-soft}` — `#b4befe`): A cooler lavender tint used inside ghost variants and hover / focus treatments.
- **Primary Deep** (`{colors.primary-deep}` — `#6A4C93`): The deeper violet used for richer emphasis and inline accent moments.

### Surface
- **Canvas** (`{colors.canvas}` — `#1F1F28`): The default indigo-black page background. The only surface mode in the brand's marketing system.
- **Canvas Soft** (`{colors.canvas-soft}` — `#1e1e2e`): A slightly lighter dark fill used inside code blocks and form inputs to mark them visually distinct against the canvas.
- **Hairline** (`{colors.hairline}` — `#313244`): 1 px solid borders — feature cards, buttons, dividers between rows. The brand's universal edge colour.
- **Hairline Soft** (`{colors.hairline-soft}` — `#45475a`): A softer slate divider tint used for secondary edges and internal separation.

### Text
- **Ink** (`{colors.ink}` — `#bac2de`): Default text colour on the dark canvas — cool, soft, and highly legible without going pure white.
- **Ink Strong** (`{colors.ink-strong}` — `#cdd6f4`): High-emphasis text for hero headlines and primary labels.
- **Body** (`{colors.body}` — `#cdd6f4`): Supporting copy and long-form body paragraphs in the default reading color.
- **Mute** (`{colors.mute}` — `#a6adc8`): Lowest-priority on-dark text — captions, fine print, footer secondary lines.
...
### Semantic
The brand doesn't surface a separate error / warning palette in its public marketing pages — the underlying Docusaurus default semantic palette exists in the design system but is reserved for in-product / docs contexts. Validation cues on the marketing surface use the primary accent for positive states and muted neutrals for missing states.
...
#### Collapsing Strategy
Nav collapses to hamburger at mobile; the menu overlay keeps the same primary CTA pinned at the bottom. Feature-card grids drop to 1-up; hero typography scales fluidly.
...
- A 2 px solid `{colors.primary}` accent border occasionally marks "featured" or "active" status on a card.
...
**`button-primary`** — the primary brand CTA.
...
**`button-ghost-green`** — text-only with brand-tinted label, for tertiary actions.
...
**`green-divider-band`** — a thin accent band that occasionally separates major sections.
...
### Do
- Reserve `{colors.primary}` (`#A387D5`) for every primary CTA, active highlight, and brand-defining accent moment. The accent is the brand's centre of gravity.
- Use the dark `{colors.canvas}` (`#1F1F28`) as the only page surface. There is no light-mode rhythm.
...
- Don't use the primary accent as a body-text fill. It's reserved for emphasis and CTA moments.
